...
class ChangweiRepository:
    ...
    async def tasks(self, project_id):
        ...
        return list(
            await self.db.scalars(
                select(ChangweiTask)
                .where(ChangweiTask.project_id == project_id)
                .order_by(ChangweiTask.updated_at.desc())
            )
        )
    ...
    async def materials(self, project_id):
        ...
        return list(
            await self.db.scalars(
                select(ChangweiMaterial)
                .options(defer(ChangweiMaterial.data))
                .where(ChangweiMaterial.project_id == project_id)
                .order_by(ChangweiMaterial.created_at.desc())
            )
        )
    ...
    async def artifacts(self, task_id):
        ...
        return list(
            await self.db.scalars(
                select(ChangweiArtifact)
                .options(defer(ChangweiArtifact.data))
                .where(ChangweiArtifact.task_id == task_id)
                .order_by(ChangweiArtifact.created_at.desc())
            )
        )
    ...
    async def audits(self, project_id):
        ...
        return list(
            await self.db.scalars(
                select(ChangweiAudit)
                .where(ChangweiAudit.project_id == project_id)
                .order_by(ChangweiAudit.created_at.desc())
                .limit(100)
            )
        )
