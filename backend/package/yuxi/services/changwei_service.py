...
TASK_TYPES = {
    ...
    'management_log': {'name': '项管日志', 'modules': ['天气与水文', '工程进度', '工程质量', '安全环保', '征地组卷报批', '合同管理', '信息管理']},
    'supervision_report': {'name': '监理月报', 'modules': ['工程施工概况', '工程进度', '质量验收与评定', '检测与测量', '安全管理', '合同与支付', '监理工作', '下月工作计划']},
    'management_report': {'name': '项管月报', 'modules': ['工程概况与组织', '人员设备配置', '安全管理', '技术质量管理', '形象进度', '合同支付与资金', '分包合同与变更', '档案与信息管理', '参建单位工作', '管理总结与下月计划']},
    'scheme_review': {'name': '施工方案审核', 'modules': ['工程概况核对', '编制依据及有效性', '主要规范完整性', '施工进度匹配', '施工工艺与设计要求', '质量保证措施', '安全措施与应急预案', '审查意见汇总']},
}
...
class ChangweiService:
    ...
    async def create_task(self, project_id, kind, title, period):
        ...
        row = ChangweiTask(id=str(uuid4()), project_id=project_id, kind=kind, title=title.strip(), period=period,
            creator_uid=self.uid, status='draft', revision=1, content={'material_ids': [], 'modules': [
    ...
    async def update_module(self, task_id, module_id, revision, text, confirm, assignee=None):
        ...
        module.update(text=text.strip(), confirmed_by=self.uid if confirm else None,
                      confirmed_at=datetime.now(timezone.utc).isoformat() if confirm else None)
        task.content = content
        task.revision += 1
        task.status = 'confirmed' if all(m.get('confirmed_by') for m in content['modules']) else 'draft'
        self.repo.audit(task.project_id, '确认模块' if confirm else '保存模块', task.id, {'module': module['name'], 'revision': task.revision})
        await self.db.commit()
        return view(task)
    ...
    async def upload(self, project_id, filename, data):
        ...
        try:
            ...
            for name, raw in files:
                ...
                row = ChangweiMaterial(id=str(uuid4()), project_id=project_id, filename=name[:255],
                    category=classify(name), parse_status=parsed['status'], confirmed=False, content=parsed, data=raw)
                self.db.add(row)
                result.append(row)
            await self.db.flush()
    ...
    async def ocr_material(self, material_id):
        ...
        content['warnings'] = [w for w in content['warnings'] if not any(w.startswith(f'第 {n} 页需要 OCR') for n in pages)]
        for item in results:
    ...
    async def draft(self, task_id, module_id, model_spec, instruction):
        ...
        for material in materials:
            if material.id in task.content.get('material_ids', []) and material.confirmed and material.content['segments']:
                for segment in material.content['segments']:
                    sources.append({'source': f"{material.filename} / {segment['location']}" + ('（解析有待核验项）' if material.parse_status != 'ready' else ''), 'text': segment['text']})
        ...
        await self.db.rollback()
        try:
            response = await asyncio.wait_for(select_model(model_spec=model_spec).call([
                {'role': 'system', 'content': prompt}, {'role': 'user', 'content': request}]), timeout=120)
        except Exception as exc:
    ...
    async def generate(self, task_id, revision, template_id=None):
        ...
        if template_id:
            ...
            try:
                data = await asyncio.to_thread(export_supervision_template, template.data, modules, task.period, self.uid)
