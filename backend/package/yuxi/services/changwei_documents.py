...
CATEGORIES = ["施工方案", "施工月报", "监理月报", "资金台账", "合同台账", "收发文台账", "检测资料", "规范依据", "其他资料"]


def classify(filename: str) -> str:
    """给出可由用户修正的文件名分类建议。"""
    for words, category in [("规范|标准|技术要求|审核依据", "规范依据"), ("资金|支付|工资", "资金台账"), ("合同", "合同台账"), ("来文|收文|发文", "收发文台账"), ("检测|检验", "检测资料"), ("方案", "施工方案"), ("监理.*月报", "监理月报"), ("月报", "施工月报")]:
...
def parse_document(filename: str, data: bytes) -> dict:
    ...
    if suffix == '.docx':
        ...
        for sheet in wb:
            ...
            for row_number, (row, values) in enumerate(zip(sheet.iter_rows(max_row=min(sheet.max_row, 20000), max_col=min(sheet.max_column, 200)), cached[sheet.title].iter_rows(max_row=min(sheet.max_row, 20000), max_col=min(sheet.max_column, 200))), 1):
                cells = []
        ...
        return {'status': 'unsupported', 'segments': [], 'warnings': ['暂不支持此格式，请转换为 DOCX、XLSX、PDF 或 UTF-8 文本']}
    ...
    return {'status': 'needs_attention' if warnings or not segments else 'ready', 'segments': bounded, 'warnings': warnings}


def normalize_station(text: str) -> str:
    """统一常见口述部位和数字桩号，保留原始事实。"""
    for source, target in [('左分洪洞', 'FHDL'), ('右分洪洞', 'FHDR'), ('左洞', 'FHDL'), ('右洞', 'FHDR'), ('首部明渠', 'SBMQ'), ('尾部明渠', 'WBMQ')]:
