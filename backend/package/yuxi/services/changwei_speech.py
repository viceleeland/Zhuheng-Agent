...
def map_speech_event(event):
    ...
    if kind == 'conversation.item.input_audio_transcription.text':
        return {'type': 'partial', 'segment_id': event['item_id'],
...
async def relay_speech(client):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.ws_connect(url, headers={'Authorization': f'Bearer {key}'},
                                          heartbeat=20, max_msg_size=1024 * 1024) as upstream:
                await send_event(upstream, 'session.update', session={
                    'input_audio_format': 'pcm', 'sample_rate': 16000,
                    'input_audio_transcription': {'language': 'zh'},
                    'turn_detection': {'type': 'server_vad', 'threshold': 0.0, 'silence_duration_ms': 600},
                })
                workers = [asyncio.create_task(receive_audio(upstream)),
                           asyncio.create_task(receive_text(upstream)), asyncio.create_task(watch_deadlines())]
        ...
        with suppress(WebSocketDisconnect, RuntimeError):
            await client.send_json({'type': 'error', 'message': '实时转录连接中断或音频无效；已完成的文字可保留，请重试'})
    finally:
