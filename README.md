...
工程流程与 Skills 分工协作：技能定义各类日志、月报和方案审查的整理方法；业务服务控制资料范围、人员权限、版本与确认；文档程序负责模板填充和归档。五类技能随服务启动注册，在 Skills 列表中以 `engineering-` 开头，可在普通 Agent 配置中选用。工作台固定使用随版本发布的技能，个人同名技能不会改变工作台规则。聊天选用技能提供草稿，工程任务的保存、确认与成果生成仍在工作台完成。
