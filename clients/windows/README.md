...
5. 运行构建目录中的 `ChangweiAgent.exe --smoke-test` 会打开真实客户端窗口，并在相邻 `smoke-evidence` 目录记录首次导航结果和页面截图。记录完成后自动正常关闭窗口。此参数只用于本机构建验收；不启用调试端口。
...
升级前请先保存草稿并关闭江擎。1.0.2 使用应用互斥标记和安装前文件替换权限探测，在旧版仍占用文件时中止安装，不强制终止应用，不清理登录数据。
